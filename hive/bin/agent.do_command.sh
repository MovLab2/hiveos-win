...
function do_command () {
	...
	case $command in
		...
		;;
		exec)
			exec=$(echo $body | jq '.exec' --raw-output | sed 's/\r$//')
		...
		;;
		config)
			config=$(echo $body | jq '.config' --raw-output | sed 's/\r$//')
			justwrite=$(echo $body | jq '.justwrite' --raw-output | sed 's/\r$//') #don't restart miner, just write config, maybe WD settings will be updated
        ...
        sed -i 's/\r$//' /tmp/rig.conf.new
				...
				if [[ $RIG_PASSWD != $NEW_PASSWD ]]; then
					...
					error=$(echo $response | jq '.error | select (.!=null)' --raw-output | sed 's/\r$//')
					[[ ! -z $error ]] && echo -e "${RED}Server error:${NOCOLOR} `echo $response | jq '.error.message' -r`" && return 1
				...
				echo "$config" > $RIG_CONF && sed -i 's/\r$//' $RIG_CONF && sync
				. $RIG_CONF

				# Save wallet if given -----------------------------------------------
				wallet=$(echo $body | jq '.wallet' --raw-output | sed 's/\r$//')
		...
		;;
		nvidia_oc)
			nvidia_oc=$(echo $body | jq '.nvidia_oc' --raw-output | sed 's/\r$//')
		...
		;;
		amd_oc)
			amd_oc=$(echo $body | jq '.amd_oc' --raw-output | sed 's/\r$//')
		...
		;;
		amd_download)
			gpu_index=$(echo $body | jq '.gpu_index' --raw-output | sed 's/\r$//')
		...
		amd_upload)
			gpu_index=$(echo $body | jq '.gpu_index' --raw-output | sed 's/\r$//')
			rom_base64=$(echo $body | jq '.rom_base64' --raw-output | sed 's/\r$//')
			if [[ -z $gpu_index || $gpu_index == "null" ]]; then
				message error "No \"gpu_index\" given"
			elif [[ -z $rom_base64 || $rom_base64 == "null" ]]; then
				message error "No \"rom_base64\" given"
			else
				force=$(echo $body | jq '.force' --raw-output | sed 's/\r$//')
		...
		;;
		openvpn_set)
			local clientconf=$(echo $body | jq '.clientconf' --raw-output | sed 's/\r$//')
			local cacrt=$(echo $body | jq '.cacrt' --raw-output | sed 's/\r$//')
			local clientcrt_fname=$(echo $body | jq '.clientcrt_fname' --raw-output | sed 's/\r$//')
			local clientcrt=$(echo $body | jq '.clientcrt' --raw-output | sed 's/\r$//')
			local clientkey_fname=$(echo $body | jq '.clientkey_fname' --raw-output | sed 's/\r$//')
			local clientkey=$(echo $body | jq '.clientkey' --raw-output | sed 's/\r$//')
			local vpn_login=$(echo $body | jq '.vpn_login' --raw-output | sed 's/\r$//')
			local vpn_password=$(echo $body | jq '.vpn_password' --raw-output | sed 's/\r$//')
