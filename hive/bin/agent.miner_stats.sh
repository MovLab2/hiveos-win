...
function miner_stats {
	...
	case $miner in
		...
		;;
		ccminer)
			threads=`echo "threads" | nc -w $API_TIMEOUT localhost 4068 | tr -dc '[:print:]'` #&& echo $threads
			if [[ $? -ne 0  || -z $threads ]]; then
				echo -e "${YELLOW}Failed to read $miner stats from localhost:4068${NOCOLOR}"
			else
				summary=`echo "summary" | nc -w 2 localhost 4068 | tr -dc '[:print:]'`
		...
		bminer) #@see https://www.bminer.me/references/
			...
			if [[ $? -ne 0 || -z $stats_raw ]]; then
				...
				local uptime=$(( $(date +%s) - $((jq '.start_time' <<< "$stats_raw") | sed 's/\r$//') ))
		...
		xmr-stak)
			...
			if [[ $? -ne 0 || -z $stats_raw ]]; then
				...
				#fan=$(jq -sc '.[0] + .[1]'  <<< "$fan [-1]")
				stats=$(jq --argjson temp "$temp" --argjson fan "$fan" --argjson cpu_temp $(echo $cpu_temp | sed 's/\r$//') '{hs: [.hashrate.threads[][0]], $temp, $fan, $cpu_temp, uptime: .connection.uptime}' <<< $stats_raw)
			fi
		;;
		xmrig)
